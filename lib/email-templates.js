...
export function approvalAffiliateEmail({ name, code }) {
  return shell({
    ...
    children: `<p style="margin:0;color:#52645d;font-size:15px;line-height:1.7">Hello ${escapeHtml(name)}, your affiliate application has been approved.</p><div style="margin:24px 0;padding:20px;background:#eaf7f2;border:1px solid #c9eadc;border-radius:12px"><div style="font-size:11px;font-weight:800;letter-spacing:1.4px;color:#128174">YOUR REFERRAL CODE</div><div style="margin-top:8px;font-size:28px;font-weight:800;letter-spacing:2px;color:#071412">${escapeHtml(code)}</div><div style="margin-top:12px;font-size:13px;color:#52645d;word-break:break-all">${appUrl()}/r/${encodeURIComponent(code)}</div></div><a href="${appUrl()}/affiliate/login" style="display:inline-block;padding:13px 20px;border-radius:8px;background:#079f78;color:#fff;text-decoration:none;font-weight:700">Open affiliate dashboard</a>`,
  });
}
