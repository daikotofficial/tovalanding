...
const appUrl = () =>
  String(process.env.APP_URL || "https://tova.com.ng").replace(/\/$/, "");
...
export function affiliatePaymentEmail({
  affiliateName,
  customerName,
  product,
  amount,
  ...
  const expiry = expiresAt
    ...
    : "Not provided by the product yet";
  return shell({
    preheader: `A referred customer just paid for ${product}.`,
    eyebrow: "COMMISSION ACTIVITY",
    title: "Your referral just paid",
    children: `<p style="margin:0;color:#52645d;font-size:15px;line-height:1.7">Hello ${escapeHtml(affiliateName)}, ${escapeHtml(customerName || "A referred customer")} has completed a qualifying payment for <strong>${escapeHtml(product)}</strong>.</p><div style="margin:24px 0;padding:20px;background:#eaf7f2;border:1px solid #c9eadc;border-radius:12px"><p style="margin:0;color:#52645d;font-size:14px">Commission accrued</p><p style="margin:6px 0 0;color:#071412;font-size:28px;font-weight:800">₦${escapeHtml(amount)}</p><p style="margin:12px 0 0;color:#52645d;font-size:14px">Subscription expiry: <strong>${escapeHtml(expiry)}</strong></p></div><a href="${appUrl()}/affiliate/dashboard" style="display:inline-block;padding:13px 20px;border-radius:8px;background:#079f78;color:#fff;text-decoration:none;font-weight:700">Open affiliate dashboard</a>`,
  });
}
