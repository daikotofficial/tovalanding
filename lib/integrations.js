...
export async function recordSignup({
  ...
  const result = await db
    .prepare(
      `INSERT INTO referrals(affiliate_id,product,external_id,referred_name,referred_email,source,status,signed_up_at,created_at)
       VALUES(?,?,?,?,?,?,'registered',?,?)
       ON CONFLICT(product,external_id) DO UPDATE SET referred_name=COALESCE(excluded.referred_name,referrals.referred_name),referred_email=COALESCE(excluded.referred_email,referrals.referred_email)
...
export async function recordSubscription({
  ...
  const outcome = await db.transaction(async () => {
    ...
    await db
      .prepare(
        "UPDATE referrals SET status='converted',converted_at=?,referred_name=COALESCE(?,referred_name),subscription_expires_at=COALESCE(?,subscription_expires_at) WHERE id=?",
      )
      .run(
        now,
    ...
    const inserted = await db
      .prepare(
        "INSERT INTO commissions(affiliate_id,referral_id,amount,rate,currency,status,created_at) VALUES(?,?,?,10,?,'pending',?) ON CONFLICT(referral_id) DO NOTHING RETURNING id",
      )
