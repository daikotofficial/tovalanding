...
export async function recordSubscription({
  ...
  const outcome = await db.transaction(async () => {
    const referral = await db
      .prepare(
        "SELECT * FROM referrals WHERE product=? AND external_id=? LIMIT 1",
      )
      .get(product.trim(), externalId.trim());
    if (!referral) return { result: { recorded: false }, notification: null };
    const productKey = product.trim();
    const eventReference = String(paymentReference || reference || "")
      .trim()
      .slice(0, 180);
    if (!eventReference) throw new Error("INVALID_PAYMENT_REFERENCE");
    const commissionBase =
      productKey === "tovabooks"
        ? Math.floor((amount * 10000) / (10000 + VAT_RATE_BPS))
        : amount;
    const commission = Math.floor((commissionBase * COMMISSION_RATE) / 100);
    ...
    const existingExpiry = referral.subscription_expires_at
      ? new Date(referral.subscription_expires_at)
      : null;
    const nextExpiry =
      expiry &&
      (!existingExpiry ||
        ...
        ? expiry.toISOString()
        : referral.subscription_expires_at || null;
    await db
      .prepare(
        "UPDATE referrals SET status='converted',converted_at=COALESCE(converted_at,?),referred_name=COALESCE(?,referred_name),referred_company=COALESCE(?,referred_company),subscription_expires_at=? WHERE id=?",
      )
      .run(
        now,
    ...
    const inserted = await db
      .prepare(
        "INSERT INTO commissions(affiliate_id,referral_id,amount,rate,currency,payment_reference,status,created_at) VALUES(?,?,?,20,?,?,'pending',?) ON CONFLICT(referral_id,payment_reference) DO NOTHING RETURNING id",
      )
      .get(
