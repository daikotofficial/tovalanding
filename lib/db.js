...
async function getPool() { if (!pool) { const sslMode = String(process.env.DATABASE_SSL || "verify").trim().toLowerCase(); pool = new Pool({ connectionString: process.env.DATABASE_URL, max: 10, idleTimeoutMillis: 30000, connectionTimeoutMillis: 10000, statement_timeout: 15000, query_timeout: 20000, ssl: sslMode === "disable" ? false : { rejectUnauthorized: sslMode !== "allow-insecure" } }); pool.on("error", (error) => console.error("PostgreSQL pool error", error)); } if (!postgresReady) postgresReady = pool.query(schema); await postgresReady; return pool; }
