...
let postgresReady;
let sqlite;
const postgresPlaceholders = (sql) => {
  let i = 0;
...
CREATE TABLE IF NOT EXISTS referrals (id BIGSERIAL PRIMARY KEY,affiliate_id BIGINT NOT NULL REFERENCES affiliates(id),product TEXT NOT NULL,external_id TEXT NOT NULL,referred_name TEXT,referred_company TEXT,referred_email TEXT,source TEXT NOT NULL DEFAULT 'link',status TEXT NOT NULL DEFAULT 'registered',signed_up_at TIMESTAMPTZ,converted_at TIMESTAMPTZ,subscription_expires_at TIMESTAMPTZ,created_at TIMESTAMPTZ NOT NULL,UNIQUE(product,external_id));
CREATE TABLE IF NOT EXISTS commissions (id BIGSERIAL PRIMARY KEY,affiliate_id BIGINT NOT NULL REFERENCES affiliates(id),referral_id BIGINT NOT NULL REFERENCES referrals(id),amount BIGINT NOT NULL CHECK(amount > 0),rate INTEGER NOT NULL DEFAULT 20,currency TEXT NOT NULL DEFAULT 'NGN',payment_reference TEXT NOT NULL DEFAULT '',status TEXT NOT NULL DEFAULT 'pending',created_at TIMESTAMPTZ NOT NULL,UNIQUE(referral_id,payment_reference));
...
function getSqlite() {
  ...
  if (!sqlite) {
    ...
    const commissionColumns = sqlite.pragma("table_info('commissions')");
    const hasPaymentReference = commissionColumns.some(
      (column) => column.name === "payment_reference",
    );
    const referralUniqueIndex = sqlite
      .pragma("index_list('commissions')")
      .find((index) => {
        if (!index.unique) return false;
        const columns = sqlite.pragma(
          `index_info('${String(index.name).replaceAll("'", "''")}')`,
        );
        return columns.length === 1 && columns[0].name === "referral_id";
      });
    if (referralUniqueIndex || !hasPaymentReference) {
      const paymentReferenceSelect = hasPaymentReference
        ? "payment_reference"
        : "''";
      sqlite.pragma("foreign_keys = OFF");
      sqlite.exec(`
        CREATE TABLE commissions_rebuilt (id INTEGER PRIMARY KEY,affiliate_id INTEGER NOT NULL REFERENCES affiliates(id),referral_id INTEGER NOT NULL REFERENCES referrals(id),amount INTEGER NOT NULL CHECK(amount > 0),rate INTEGER NOT NULL DEFAULT 20,currency TEXT NOT NULL DEFAULT 'NGN',payment_reference TEXT NOT NULL DEFAULT '',status TEXT NOT NULL DEFAULT 'pending',created_at TEXT NOT NULL,UNIQUE(referral_id,payment_reference));
        INSERT INTO commissions_rebuilt (id,affiliate_id,referral_id,amount,rate,currency,payment_reference,status,created_at) SELECT id,affiliate_id,referral_id,amount,rate,currency,${paymentReferenceSelect},status,created_at FROM commissions;
    ...
    try {
      sqlite.exec("ALTER TABLE referrals ADD COLUMN referred_company TEXT");
    } catch (error) {
...
async function getPool() {
  if (!pool) {
    const sslMode = String(process.env.DATABASE_SSL || "verify")
      .trim()
      .toLowerCase();
    pool = new Pool({
      ...
      ssl:
        sslMode === "disable"
          ? false
          : { rejectUnauthorized: sslMode !== "allow-insecure" },
    });
  ...
  if (!postgresReady)
    postgresReady = pool
      ...
      .then(() =>
        pool.query(
          "ALTER TABLE referrals ADD COLUMN IF NOT EXISTS referred_name TEXT; ALTER TABLE referrals ADD COLUMN IF NOT EXISTS referred_company TEXT; ALTER TABLE referrals ADD COLUMN IF NOT EXISTS subscription_expires_at TIMESTAMPTZ; ALTER TABLE commissions ADD COLUMN IF NOT EXISTS payment_reference TEXT NOT NULL DEFAULT ''; ALTER TABLE commissions DROP CONSTRAINT IF EXISTS commissions_referral_id_key; CREATE UNIQUE INDEX IF NOT EXISTS commissions_referral_payment_unique ON commissions(referral_id,payment_reference); UPDATE commissions c SET amount=ROUND(c.amount * 200.0 / 107.5), rate=20 WHERE c.rate=10 AND NOT EXISTS (SELECT 1 FROM payout_commissions pc WHERE pc.commission_id=c.id);",
        ),
      );
  await postgresReady;
  return pool;
...
async function execute(sql, args = []) {
  const context = transactionStore.getStore();
  if (usePostgres)
    return (context?.client || (await getPool())).query(
      postgresPlaceholders(sql),
      args,
    );
  const statement = getSqlite().prepare(sql);
  if (/^\s*(SELECT|WITH|PRAGMA)/i.test(sql) || /\bRETURNING\b/i.test(sql))
    return { rows: statement.all(...args), rowCount: 0 };
...
function prepare(sql) {
  return {
    async get(...args) {
      return (await execute(sql, args)).rows[0] || undefined;
    },
    async all(...args) {
