...
CREATE TABLE IF NOT EXISTS referrals (id BIGSERIAL PRIMARY KEY,affiliate_id BIGINT NOT NULL REFERENCES affiliates(id),product TEXT NOT NULL,external_id TEXT NOT NULL,referred_name TEXT,referred_email TEXT,source TEXT NOT NULL DEFAULT 'link',status TEXT NOT NULL DEFAULT 'registered',signed_up_at TIMESTAMPTZ,converted_at TIMESTAMPTZ,subscription_expires_at TIMESTAMPTZ,created_at TIMESTAMPTZ NOT NULL,UNIQUE(product,external_id));
...
async function getPool() {
  ...
  if (!postgresReady)
    postgresReady = pool
      ...
      .then(() =>
        pool.query(
          "ALTER TABLE referrals ADD COLUMN IF NOT EXISTS referred_name TEXT; ALTER TABLE referrals ADD COLUMN IF NOT EXISTS subscription_expires_at TIMESTAMPTZ; UPDATE commissions c SET amount=ROUND(c.amount * 200.0 / 107.5), rate=20 WHERE c.rate=10 AND NOT EXISTS (SELECT 1 FROM payout_commissions pc WHERE pc.commission_id=c.id);",
        ),
      );
  await postgresReady;
  return pool;
