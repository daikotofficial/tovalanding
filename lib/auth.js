...
export function verifyPassword(password, storedHash, storedSalt) {
  ...
  const actual = scryptSync(password, Buffer.from(storedSalt, "hex"), 64, {
    N: 16384,
    r: 8,
    p: 1,
  });
...
export function checkOrigin(req) {
  ...
  // Some managed proxies remove Origin from same-origin form/fetch requests.
  // A same-origin Referer is an equivalent CSRF signal for those requests.
  if (!origin) {
  ...
  for (const value of configured) {
    try {
      ...
      // The landing site intentionally redirects www to the apex domain, but
      // both hosts may still originate a browser request during propagation.
  ...
  if (!allowed.has(origin)) throw new Error("ORIGIN");
}
...
export async function limit(key, maximum = 5, duration = 600000) {
  const now = Date.now();
  return db.transaction(async () => {
    ...
    await db.prepare(upsert).run(key, now + duration);
    return true;
  });
}
export async function challenge(email, purpose = "verification") {
  ...
  try {
    await sendMail({
      ...
      subject:
        purpose === "password-reset"
          ? "Reset your TovaERP affiliate password"
          : "Verify your TovaERP affiliate account",
      html: verificationEmail(code, purpose),
      text:
        "Your code is " +
        code +
    ...
    await db
      .prepare("DELETE FROM auth_challenges WHERE email=? AND code_hash=?")
      .run(email, digest);
    throw error;
  }
}
export async function consume(email, code) {
  ...
  return db.transaction(async () => {
    ...
    await db
      .prepare("UPDATE auth_challenges SET attempts=attempts+1 WHERE email=?")
      .run(email);
    if (
      !timingSafeEqual(
      ...
      return false;
    await db.prepare("DELETE FROM auth_challenges WHERE email=?").run(email);
    return true;
  });
}
...
export async function createSession(response, id) {
