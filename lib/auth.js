  ...
  for (const value of configured) {
    try {
      ...
      if (parsed.hostname.startsWith("www.")) {
        allowed.add(`${parsed.protocol}//${parsed.hostname.slice(4)}${parsed.port ? `:${parsed.port}` : ""}`);
      } else {
        allowed.add(`${parsed.protocol}//www.${parsed.hostname}${parsed.port ? `:${parsed.port}` : ""}`);
      }
    } catch {
      continue;
    }
...
export async function challenge(email, purpose = "verification") {
  ...
  try {
    await sendMail({
      to: email,
      subject: purpose === "password-reset" ? "Reset your TovaERP affiliate password" : "Verify your TovaERP affiliate account",
    ...
    await db.prepare("DELETE FROM auth_challenges WHERE email=? AND code_hash=?").run(
      email,
      digest,
    );
    throw error;
