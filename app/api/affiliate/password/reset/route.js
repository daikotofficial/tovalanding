...
export async function POST(req) {
  try {
    ...
    if (!consume(email, String(data.verification || ""))) return failure("This code is invalid or expired.", 401);
    const user = db.prepare("SELECT id,status FROM affiliates WHERE email=? AND status IN ('pending','active')").get(email);
    if (!user) return failure("This code is invalid or expired.", 401);
    const password = createPassword(data.newPassword);
    db.transaction(() => {
      db.prepare("UPDATE affiliates SET password_hash=?,password_salt=?,password_updated_at=?,verified=1,failed_login_attempts=0,locked_until=0 WHERE id=?").run(password.hash, password.salt, new Date().toISOString(), user.id);
