...
export async function POST(req) {
  ...
  try {
    ...
    const credentials = await db.prepare("SELECT password_hash,password_salt FROM affiliates WHERE id=?").get(user.id);
    ...
    await db.prepare("UPDATE affiliates SET password_hash=?,password_salt=?,password_updated_at=?,failed_login_attempts=0,locked_until=0 WHERE id=?").run(password.hash, password.salt, new Date().toISOString(), user.id);
