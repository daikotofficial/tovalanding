...
export async function POST(req) {
  try {
    ...
    if (!limit("password-reset:" + email, 3)) return failure("Please wait before requesting another code.", 429);
    if (db.prepare("SELECT id FROM affiliates WHERE email=?").get(email)) await challenge(email);
