...
export async function POST(req) {
  try {
    ...
    if (!limit("login:" + email, 8)) return failure("Too many login attempts. Please try again later.", 429);
    const hasVerification = Boolean(data.verification);
    const valid = hasVerification ? consume(email, String(data.verification)) : verifyPassword(data.password, user.password_hash, user.password_salt);
    if (!valid) {
      const attempts = user.failed_login_attempts + 1;
      db.prepare("UPDATE affiliates SET failed_login_attempts=?,locked_until=? WHERE id=?").run(attempts, attempts >= 5 ? Date.now() + 15 * 60 * 1000 : 0, user.id);
      return failure("Invalid email or password.", 401);
    }
    if (hasVerification && !user.password_hash) {
      ...
      db.prepare("UPDATE affiliates SET password_hash=?,password_salt=?,password_updated_at=? WHERE id=?").run(password.hash, password.salt, new Date().toISOString(), user.id);
    }
    db.prepare("UPDATE affiliates SET verified=1,verification_code='',failed_login_attempts=0,locked_until=0 WHERE id=?").run(user.id);
