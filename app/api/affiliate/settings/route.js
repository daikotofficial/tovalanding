...
export async function GET() {
  ...
  const settings = await db.prepare("SELECT name,email,phone,payout_method,payout_account_name,payout_account_number,payout_bank_name FROM affiliates WHERE id=?").get(user.id);
  const payout = {
  ...
  if (settings.payout_account_name && !String(settings.payout_account_name).startsWith("enc:v1:")) {
    await db.prepare("UPDATE affiliates SET payout_account_name=?,payout_account_number=?,payout_bank_name=? WHERE id=?").run(encryptPayoutValue(payout.accountName), encryptPayoutValue(payout.accountNumber), encryptPayoutValue(payout.bankName), user.id);
  }
...
export async function POST(req) {
  ...
  try {
    ...
    await db.prepare("UPDATE affiliates SET payout_method='bank_transfer',payout_account_name=?,payout_account_number=?,payout_bank_name=? WHERE id=?").run(encryptPayoutValue(accountName), encryptPayoutValue(accountNumber), encryptPayoutValue(bankName), user.id);
